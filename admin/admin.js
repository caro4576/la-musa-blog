...
const editarEscrituraTitulo = document.querySelector("#editar-escritura-titulo");
const editarEscrituraCategoria = document.querySelector("#editar-escritura-categoria");
const editarEscrituraContenido = document.querySelector("#editar-escritura-contenido");
const cancelarEdicionEscritura = document.querySelector("#cancelar-edicion-escritura");
...
formEditarEscritura.addEventListener("submit", event => {
    ...
    fetch(`http://localhost:3000/api/escrituras/${escrituraEditandoId}`, {
        method: "PUT",
        headers: {
            "Content-Type": "application/json"
        },
...
const editarLibroDescripcion = document.querySelector("#editar-libro-descripcion");
        ...
        .then(libros => {
            ...
            libros.forEach(libro => {
                ...
                elemento.textContent =
                    `${libro.titulo} — ${libro.fecha || "Sin fecha"} — ${libro.descripcion || "Sin descripción"} — ${libro.enlace || "Sin enlace"}`;
                ...
                botonEliminar.addEventListener("click", () => {
                    ...
                    fetch(
                        `http://localhost:3000/api/libros/${libro._id}`,
                        {
                            method: "DELETE"
                        }
                    )
...
formLibro.addEventListener("submit", event => {
    ...
    const titulo =
        document.querySelector("#libro-titulo").value;

    const descripcion =
        document.querySelector("#libro-descripcion").value;

    const fecha =
        document.querySelector("#libro-fecha").value;

    const enlace =
        document.querySelector("#libro-enlace").value;
...
formEditarLibro.addEventListener("submit", event => {
    ...
    fetch(
        `http://localhost:3000/api/libros/${libroEditandoId}`,
        {
            method: "PUT",
